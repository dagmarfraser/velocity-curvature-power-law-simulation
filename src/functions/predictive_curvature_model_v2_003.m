function out = predictive_curvature_model_v2_003(x, y, fs, params)
% PREDICTIVE_CURVATURE_MODEL_V2_003  Anticipatory speed with goal-distance modulator.
%
%   out = predictive_curvature_model_v2_003(x, y, fs) runs with defaults.
%   out = predictive_curvature_model_v2_003(x, y, fs, Name, Value, ...) overrides params.
%
%   Implements Jen Cook's full original vision (PCM_design_v002.md):
%     FIX 1  Integrated curvature drives speed (v2_001 correction retained).
%     FIX 2  Power-law speed, commensurable with Lacquaniti beta (v2_001 retained).
%     FIX 3  Causal forward model only (v2_001 retained).
%     NEW D2 Goal-distance speed modulator: v *= exp(-goalDecay * dGoalNorm).
%            dGoalNorm = 1 - cumArc/totalArc in [0,1]; ~1 at trial start, ~0 at end.
%            goalDecay > 0: anticipatory deceleration (slow when far from goal).
%            goalDecay < 0: Fitts-law deceleration (slow when near goal).
%            goalDecay = 0 (default): no effect; identical to v2_002.
%     BUG FIX: alphaFutureTrace pre-allocated (was growing dynamically in v2_002).
%
%   Parameters D1 (memoryGain), D3 (reliabilityGain), D5 (lambda0) already exist
%   and are freed in fitPCMToHickman_v003. No model change required for those.
%
...
%       the recovered exponent.
    ...
    %% Goal-distance trace (D2 — Jen's vision, PCM_design_v002.md)
    % dGoalNorm(t) = 1 - cumArc(t)/totalArc: ~1 at trial start, ~0 at trial end.
    % Uses EBR-smoothed speed for arc integration (preferable to raw position diffs).
    % goalDecay=0 (default) leaves speed law identical to v2_002.
    % For cyclic data, pre-segment into single cycles before calling.
    arcStep   = hypot(vx, vy) .* dt;
    cumArc    = [0; cumsum(arcStep(1:end-1))];
    totalArc  = cumArc(end) + arcStep(end);
    dGoalNorm = 1 - cumArc ./ max(totalArc, epsP);   % N-by-1, monotone decreasing [0,1]
    ...
    for t = 1:tEnd
        ...
        % FIX 1 + FIX 2 + D2: integrated curvature drives power-law speed,
        % modulated by goal distance (goalDecay=0 recovers v2_002 exactly).
        goalFactor   = exp(-params.goalDecay * dGoalNorm(t));
        speedPred(t) = params.vmax * kEff^(-params.beta) * reliability(t) * goalFactor;
