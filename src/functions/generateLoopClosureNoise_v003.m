function [nMaj, nMin] = generateLoopClosureNoise_v003(model, srcMaj, srcMin, FS, alphaMaj, alphaMin)
...
%
% v003: stress-test variant — alpha range guards removed from 'xu' branch
%   and shapeXu_local. Calls generateCustomNoise_v004/XuNoise_v002 which are
%   also unconstrained. Numerical failures surface as NaN/Inf in output and
%   are caught by the caller's try-catch. Do NOT use in production loop
%   closure runs; use v002 there.
%
% Fraser, D.S. (2026)
    ...
    switch model
        ...
        case 'xu'
            ...
            M        = numel(srcMaj);
            sigMaj  = std(srcMaj, 0, 1);
            sigMin  = std(srcMin, 0, 1);
            nMaj = generateCustomNoise_v004(M, alphaMaj, sigMaj, FS);
            nMin = generateCustomNoise_v004(M, alphaMin, sigMin, FS);
...
% real-signal spectrum. real() guards residual numerical asymmetry.
    ...
    base  = generateCustomNoise_v004(M, alpha, sig, FS);  % Xu fractional-phase carrier
    Fb    = fft(base);
