function out = predictive_curvature_model_v2_004(x, y, fs, params)
% PREDICTIVE_CURVATURE_MODEL_V2_004  Adds per-cycle and kappa-peak sub-goal modes (D2).
%
%   Adds to v2_003:
%     NEW goalDecayMode  Selects the dGoalNorm computation geometry:
%       'linear'      Original monotone trial-arc fraction (was null — kept for reference).
%       'kappa_peak'  Option B: normalised arc to the next curvature peak (sawtooth at κ tips).
%                     Tests Jen's "if you can see sharp curvature coming up, you slow down."
%                     Fast oscillatory signal identifiable against the power-law background.
%       'cycle'       Option A: per-cycle reset at κ peaks. dGoalNorm = 1 − arc_within_cycle
%                     / arc_per_cycle. Sawtooth resetting at each κ peak.
%
%   Both 'kappa_peak' and 'cycle' use the same κ peak detection; they differ only in
%   how dGoalNorm is computed from those peaks. Run both to compare.
%
%   goalDecay=0 (default) disables the mechanism regardless of mode.
%
