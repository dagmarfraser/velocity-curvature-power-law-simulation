function out = predictive_curvature_model_v2_005(x, y, fs, params)
% PREDICTIVE_CURVATURE_MODEL_V2_005  Adds D15: curvature-volatility speed modulator.
%
%   Adds to v2_004:
%     NEW volatilityGain  Scales the ω-driven speed reduction (D15, Jen Cook 2026-06-04).
%       v(t) is multiplied by 1/(1 + volatilityGain·ωNorm(t)), where
%       ωNorm = ω/(ω+omegaScale) is the soft-normalised curvature volatility.
%       volatilityGain=0 (default) recovers v2_004 exactly.
%
%   Physical motivation: ω = EMA(|Δκ/Δt|) captures GEOMETRIC demand — how rapidly
%   the path is changing its bending. A stretch of locally increasing curvature
%   requires anticipatory braking beyond what σ (the statistical uncertainty channel)
%   provides. This separates two mechanisms:
%     σ  → narrows temporal horizons (noisy motor output → look further ahead)
%     ω  → scales speed down (demanding geometry ahead → slow for it directly)
%   DA prediction: PLAC > HALO on fitted volatilityGain (reduced DA → less geometric
%   anticipation), separable from reliabilityGain (which responds to σ not ω).
%
%   D2 goalDecayMode inherited unchanged from v2_004 (null; kept for reference).
%   D1 (memoryGain), D3 (reliabilityGain), D5 (lambda0) unchanged.
%   NEW output field: out.omegaNorm (normalised volatility trace, N×1).
%
