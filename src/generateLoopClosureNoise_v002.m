...
%
% MODEL = 'bootstrap'  (NEW in v002):
%   srcMaj/srcMin are the PCA-axis residual TIME SERIES (real column vectors).
%   The SAME random circular shift is applied to both axes. alpha args unused.
%
...
%   invariant — cross-axis phase coherence is preserved exactly. Every
%   per-axis spectral statistic (alpha, sigma, spectral break, non-Gaussianity)
%   is also preserved, since the operation is a permutation of samples.
%   This is the most noise-faithful available surrogate: the only assumption
%   is approximate within-trial stationarity (reasonable for 2-3 s trials).
%   No IRASA required.
%
%   Purpose: empirically quantifies the EIV cross-axis phase-coherence effect
%   (Finding #79). The gap between shaped_xu beta_gen* and bootstrap beta_gen*
%   measures how much cross-axis phase independence inflates the estimate.
%
% The four models span the design space:
...
%   shaped_xu : empirical fine structure YES, correct alpha YES (phase target)
%   bootstrap : all per-axis properties EXACT, cross-axis coherence EXACT
%
% INPUTS:
%   model     - 'fftnoise' | 'xu' | 'shaped_xu' | 'bootstrap'
%   srcMaj    - fftnoise: fft(resid_maj) [complex Mx1]
%               xu/shaped_xu/bootstrap: resid_maj [real Mx1]
%   srcMin    - fftnoise: fft(resid_min) [complex Mx1]
%               xu/shaped_xu/bootstrap: resid_min [real Mx1]
%   FS        - sampling rate (Hz) — used by xu/shaped_xu only
