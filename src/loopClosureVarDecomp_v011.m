function out = loopClosureVarDecomp_v011(opts)
% loopClosureVarDecomp_v011  Adds N_REPS=200 confirmatory-corpus support to
% v010. This is a SURGICAL fork, not a rewrite: v010's entire 795-line
% cluster-bootstrap/synthesis machinery (everything from "Two-stage cluster
% bootstrap" onward, explicitly marked "UNCHANGED FROM v008" in v010's own
% comments) is copied verbatim, untouched. The only functional change is in
% loadResults_local, which now branches on the new opts.UseConfirmatoryCorpus
% flag to select loopClosureResults_<tag>_all_shaped_xu_v013.mat (v014 for
% Cook_ASD) instead of the legacy _v007/_v008 files -- the exact per-dataset
% version mapping compare42CellVerdict_v001.m already uses and has already
% been run and validated against Finding #160 (4+6+32 self-check passed).
%
% A new validateLoadedResults_local check is added after each dataset load
% (not present in v010): getRowVec_local silently returns an all-NaN row on
% a field-name or field-size mismatch rather than erroring, which is a
% reasonable risk for v010's own stable, long-used v007/v008 corpus but not
% something to inherit silently on files (v013/v014) this pooling script has
% never been run against before. Fails loud if a dataset's finite-fraction on
% betaGenStar looks nothing like its own legacy-corpus fraction.
%
% VALIDATION REQUIRED BEFORE TRUSTING THE N=200 BRANCH: run this with
% UseConfirmatoryCorpus=false first and confirm it exactly reproduces v010's
% own already-published k=3 HKSJ number (0.3102 +/- 0.0129, [0.2548, 0.3656],
% t, df=2, Finding #140) -- NBoot's rng seed is fixed (20260619), so this
% should be an EXACT reproduction, not an approximate one. If it does not
% match exactly, the fork has a bug and the N=200 branch must not be trusted
% until that's found and fixed.
%
% Fraser, D.S. (2026)  v011
%
% ---- v010's own original header follows, unchanged ----
%
...
% Fraser, D.S. (2026)  v010
    ...
    arguments
        ...
        opts.UseConfirmatoryCorpus (1,1) logical = false
    end

    if numel(unique(opts.PipelineSubset)) ~= numel(opts.PipelineSubset)
        error("loopClosureVarDecomp_v011:duplicateSubsetEntries", "%s", ...
            "PipelineSubset contains duplicate pipeline indices.");
    end
    if numel(opts.PipelineSubset) < 2
        error("loopClosureVarDecomp_v011:subsetTooSmall", "%s", ...
    ...
    corpusTag = "N20";
    if opts.UseConfirmatoryCorpus, corpusTag = "N200"; end
    outFile = fullfile(srcDir, sprintf("loopClosureVarDecomp_v011_%s_%s.mat", subsetTag, corpusTag));
    ...
    for k = 1:nDS
        tag   = DATASETS{k,1};
        label = DATASETS{k,2};
        r = loadResults_local(srcDir, tag, opts.UseConfirmatoryCorpus);
        validateLoadedResults_local(r, label, tag, opts.UseConfirmatoryCorpus);
        rAll{k} = r;
    ...
    meta = struct("NBoot", opts.NBoot, "NInner", opts.NInner, "RngSeed", opts.RngSeed, ...
        ...
        "source", sourceDescr_local(opts.UseConfirmatoryCorpus), ...
        "useConfirmatoryCorpus", opts.UseConfirmatoryCorpus, ...
        ...
        "methodChange", "v011: adds N_REPS=200 confirmatory-corpus loading via a new opts.UseConfirmatoryCorpus flag, isolated entirely to loadResults_local. Cluster-bootstrap/synthesis machinery is byte-identical to v010 (Finding #136).");
...
function r = loadResults_local(srcDir, tag, useConfirmatory)
% v011: branches on useConfirmatory. false (default) reproduces v010
% exactly (v007/v008 legacy corpus). true selects the N_REPS=200
% confirmatory corpus, using the EXACT per-dataset version mapping
% compare42CellVerdict_v001.m already uses and has already validated
% (4+6+32 self-check passed against Finding #160) -- not re-derived here.
    if useConfirmatory
        % Matches compare42CellVerdict_v001.m's newVersion mapping exactly:
        % {'Zarandi','Cook_CTRL','Cook_ASD','Dhieb','Hickman_PLAC','Hickman_HALO','Fraser'}
        % -> {'v013',  'v013',    'v014',    'v014', 'v013',        'v013',        'v013'}
        % Dhieb is not a member of this script's DATASETS list at all
        % (excluded from pooling per Finding #140's own header), so no
        % mapping entry is needed for it here.
        confirmVersion = containers.Map( ...
            {'Cook_CTRL','Cook_ASD','Hickman_PLAC','Hickman_HALO','Fraser','Zarandi'}, ...
            {'v013',     'v014',    'v013',         'v013',        'v013',  'v013'});
        if ~isKey(confirmVersion, char(tag))
            error("loopClosureVarDecomp_v011:noConfirmVersionMapping", "%s", ...
                sprintf("No N=200 corpus version mapping for dataset tag '%s'.", tag));
        end
        ver = confirmVersion(char(tag));
        matFile = fullfile(srcDir, sprintf("loopClosureResults_%s_all_shaped_xu_%s.mat", tag, ver));
    elseif tag == "Fraser"
    ...
    if ~isfile(matFile)
        error("loopClosureVarDecomp_v011:notFound", "%s", ...
...
function validateLoadedResults_local(r, label, tag, useConfirmatory)
% NEW in v011, not present in v010. getRowVec_local silently returns an
% all-NaN row on a field-name or field-size mismatch rather than erroring
% -- a reasonable risk to accept on v010's own long-stable v007/v008
% corpus, not something to inherit silently the first time this pooling
% script is ever pointed at the v013/v014 confirmatory files. Fails loud
% if betaGenStar looks structurally wrong for this dataset, rather than
% letting a silent all-NaN column quietly flow into the bootstrap and
% produce a plausible-looking but meaningless number.
    N = numel(r);
    if N == 0
        error("loopClosureVarDecomp_v011:emptyResults", "%s", ...
            sprintf("Dataset '%s' (tag '%s', confirmatory=%d) loaded zero trials.", ...
            label, tag, useConfirmatory));
    end
    if ~isfield(r, "betaGenStar")
        error("loopClosureVarDecomp_v011:missingField", "%s", ...
            sprintf("Dataset '%s' (tag '%s'): results struct has no 'betaGenStar' field.", ...
            label, tag));
    end
    finiteFrac = mean(arrayfun(@(x) any(isfinite(x.betaGenStar(:))), r));
    if finiteFrac < 0.05
        error("loopClosureVarDecomp_v011:allNaNSuspected", "%s", sprintf( ...
            "Dataset '%s' (tag '%s', confirmatory=%d): only %.1f%% of %d trials have " + ...
            "ANY finite betaGenStar value across pipelines. This is consistent with " + ...
            "getRowVec_local silently NaN-filling every row due to a field-name or " + ...
            "field-size mismatch against this corpus version, not with genuine non-" + ...
            "convergence. Stopping rather than propagating a meaningless bootstrap.", ...
            label, tag, useConfirmatory, 100*finiteFrac, N));
    end
end

function s = sourceDescr_local(useConfirmatory)
    if useConfirmatory
        s = "N_REPS=200 confirmatory corpus: loopClosureResults_<tag>_all_shaped_xu_v013.mat (Cook_ASD: v014), matching compare42CellVerdict_v001.m's own version mapping.";
    else
        s = "loopClosureResults_<tag>_all_shaped_xu_v007.mat (Fraser: _all_shaped_xu_v008.mat) -- legacy N_REPS=20 corpus, byte-identical to v010's source.";
    end
end
